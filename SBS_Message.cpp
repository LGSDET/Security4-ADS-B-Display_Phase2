...
bool SBS_Message_Decode( char *msg,
    TADS_B_Aircraft* (*FindAircraft)(uint32_t addr),   // addr로 객체를 찾는 함수 포인터
    TADS_B_Aircraft* (*CreateAircraft)(uint32_t addr, int spriteImage),  // 새로운 항공기 객체 생성 함수
    int spriteImage                                   // 다음에 할당할 sprite 번호
    )
{
